...
# Balandligi cheklangan sifat uchun format (progressive-first = tez, h264 = iPhone-mos)
...
# 4 xil sifat. "max" = mavjud ENG YUQORI (1080p'dan balandroq bo'lsa — o'shani,
# masalan 4K/2K; ffmpeg smart re-encode iPhone mosligini kafolatlaydi).
QUALITY_FORMATS = {
    "max": (
    ...
    "low": _fmt(480),     # ⚡ 480p (eng tez)
}
QUALITY_LABELS = {"max": "Eng yuqori", "high": "1080p", "mid": "720p", "low": "480p"}


def quality_keyboard() -> InlineKeyboardMarkup:
    """4 xil sifatni tanlash tugmalari."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🌟 Eng yuqori", callback_data="igq:max"),
...
HELP = (
    ...
    "2️⃣ Sifatni tanlang: 🌟 Eng yuqori / 🎬 1080p / 📱 720p / ⚡ 480p.\n"
...
async def on_text(message: Message):
    ...
    # Linkni saqlaymiz va sifat tanlashni so'raymiz (yuklash callback'da)
    PENDING[message.from_user.id] = (url, message.message_id)
    await message.answer(
        "🎚 <b>Sifatni tanlang:</b>\n"
        "🌟 Eng yuqori · 🎬 1080p · 📱 720p · ⚡ 480p",
...
async def on_quality(call: CallbackQuery):
    ...
    # Music bot kabi TOZALASH: status xabari download_and_send ichida o'chadi;
...
def _build_opts(tmpdir: str, fmt: str) -> dict:
    """yt-dlp sozlamalarini quradi. Format (fmt) — tanlangan sifat.

    Har sifatda avval progressive MP4 (birlashtirishsiz, tez, iPhone-mos H.264)
    tanlanadi; faqat progressive bo'lmasa video+audio birlashtiriladi.
    """
    opts = {
        "format": fmt,
