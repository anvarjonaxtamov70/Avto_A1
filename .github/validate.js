/* =============================================================
   AVTO A1 — PR avtomatik tekshiruvi
   ...
   Xato topilsa exit(1) — GitHub'da PR'ga qizil ✗ chiqadi.
   ============================================================= */
const fs = require('fs');
...
try {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    const tmp = path.join(os.tmpdir(), `avto_block_${count}.js`);
...
function collectFiles(dir, acc) {
  for (const name of fs.readdirSync(dir)) {
    ...
    // Faqat kod fayllari (html/js) va kengaytmasiz fayllar (eski HTML nusxalari kabi).
    // SVG/CSS/JSON skip qilinadi — ularda noto'g'ri ishlamasligi uchun.
